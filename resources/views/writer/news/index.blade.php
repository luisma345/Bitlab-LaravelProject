...
@section('h1','Mis Noticias:')


@section('content')
    <div class="flex justify-center mt-4">
        @include('partials.ui.linkButton', ['label' => 'Crear Noticia', 'url' => route('writer.news.create')])
    </div>
    <div class="flex justify-center my-2">
        <div class="block">
            <form action="{{ route('writer.news.index') }}" method="GET">
    ...
    @else
        @if (!is_null(request()->keyword))
            ...
            <div class="flex justify-center mt-2">
                <span class="">Cantidad de resultados: {{$news->total() }}</span>
            </div>                
        @endif
        @foreach($news as $item)
            <div class="md:flex md:justify-center my-2">
                <div class="w-full md:w-2/3">
                    <a href="{{ route('writer.news.show', $item->id) }}" 
